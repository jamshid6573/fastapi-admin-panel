...
function ItemList() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      // Извлекаем detail из ответа сервера
  ...
  const openModal = (item?: Item) => {
    if (item) {
      ...
      setNewItem({
        ...
        rarity_id: rarities?.[0]?.id || 0,
        category_id: categories?.[0]?.id || 0,
        collection_id: collections?.[0]?.id || 0,
      });
    }
    setPhotoFile(null);
    setErrorMessage(null); // Очищаем ошибку при открытии модального окна
    setIsModalOpen(true);
  };
  ...
  const closeModal = () => {
    ...
    setPhotoFile(null);
    setErrorMessage(null); // Очищаем ошибку при закрытии
  };
  ...
  if (itemsLoading || typesLoading || raritiesLoading || categoriesLoading || collectionsLoading) {
